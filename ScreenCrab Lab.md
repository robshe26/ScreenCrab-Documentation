...
3. Monitor Connection: Connect a second HDMI cable from the HDMI out port on the Screen Crab to the external monitor.

5. Power Up: Insert the MicroSD card and provide power to the Screen Crab. Ensure the video signal successfully passes through to the monitor, appearing normal to the end-user.
...
## Step 2: Remote Configuration and C2
...
2. Cloud C2: Link the Screen Crab to your Hak5 Cloud C2 instance.
...
1. Display Signature Analysis: Open the display settings on the target computer. Check the connected monitor's hardware profile. Note any discrepancies, such as a generic display interface name replacing the actual monitor brand, or an unexplained restriction in the available screen refresh rates.

2. Network Monitoring: Analyze the local wireless network traffic. Identify outbound connections originating from the device communicating with the external Cloud C2
...
3. Recommendations for mitigating physical hardware attacks in a standard office environment. 
